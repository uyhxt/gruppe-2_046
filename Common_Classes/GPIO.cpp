...
bool GPIO::read()
{
     ...
     *            which means "false"
     */
    return (GPIOPinRead(gpioPortBase, gpioPin) & 0xff);
}
...
void GPIO::write(bool state)
{
     ...
     */
    GPIOPinWrite(gpioPortBase, gpioPin, (state * gpioPin));
}
...
void GPIO::setCurrent(uint32_t current)
{
    ...
    {
        gpioSys->error(GPIOWrongCurrent, &current);
    }
     ...
     */
    gpioCurrent = gpioHalfCurrentToParam[current / 2];
    refreshConfig();
}
